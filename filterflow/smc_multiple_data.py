...
class SMCMultipleData(Module):
    def __init__(self, observation_model: ObservationModelBase, transition_model: TransitionModelBase,
                 ...
                 resampling_method: ResamplerBase, name='SMCMultipleData'):
        super(SMCMultipleData, self).__init__(name=name)
    ...
    def _return(self, initial_state: State, observation_series: tf.data.Dataset, n_observations: tf.Tensor,
        ...
        def body(state, states, i, seed):
            ...
            newstate = self.update(state, observation, inputs, seed1, seed2)
            states = states.write(i, newstate)
            return newstate, states, i + 1, seed
