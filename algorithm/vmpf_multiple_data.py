...
import numpy as np
class VariationalMarginalParticleFilterMultipleData(Algorithm):
    ...
    def init(self, y0, N = None):
        if isinstance(self.proposal, DNN4TMC):
            dist = self.proposal.vmpf_proposal(0,None,y0,None)
        else:
    ...
    def step(self,x,w,w_bar,y_t,N = None):
        dist = self.proposal.vmpf_proposal(None, x, y_t, w, self.reparameterize)
        xt = dist.sample((N))

        logf = tf.reduce_logsumexp(w + self.ssm.logf(None, tf.expand_dims(xt, axis=1), x),
                                   axis=-1) - tf.reduce_logsumexp(w)
